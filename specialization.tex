...
\begin{remark} \label{rem:scalars-on-ext} Let
	...
	\[ \sesarrows{\mc{F}}{f}{\mc{E}}{ag}{\mc{G}}. \] To see this, consider the two exact sequences
	\[\begin{tikzcd}
		0 \arrow{r} &
		\mc{F} \arrow{r}{f} \arrow[swap]{d}{a} &
		\mc{E} \arrow{r}{g} \arrow[equal]{d} &
		\mc{G} \arrow{r} \arrow[equal]{d} &
		0 \\
		0 \arrow{r} & \mc{F} \arrow[swap]{r}{a^{-1}f} & \mc{E} \arrow{r}{g} & \mc{G} \arrow{r} & 0.
	\end{tikzcd} \]
	...
	But the lower sequence is equivalent to the sequence \[0\to \mc{F} \xlongrightarrow{f} \mc{E} \xlongrightarrow{ag} \mc{G} \to 0,\] hence our claim holds.
\end{remark}

\begin{example}
	The vector bundle $\mc{O}(1)\oplus \mc{O}(1)$ on $\PP^1$ specializes to $\mc{O}\oplus \mc{O}(2)$.
	This can be seen as follows.
	Consider $\Ext^1(\mc{O}(2),\mc{O})$, whose elements correspond to extensions of the form
	\[ \ses{\mc{O}}{\mc{E}}{\mc{O}(2)}\]
	up to equivalence, the zero element corresponding to the split extension $\mc{E} = \mc{O} \oplus \mc{O}(2)$.
	...
	such that for all nonzero rational points $\xi\in\AA^1 = \VV(\Ext^1(\mc{O}(2),\mc{O})\dual)$ we have $\mc{E}_{\text{univ}}|_{\xi \times \AA^1} \simeq \mc{O}(1)\oplus\mc{O}(1)$ and $\mc{E}_{\text{univ}}|_{0\times \AA^1} \simeq \mc{O} \oplus \mc{O}(2)$. Note that $\mc{E}_{\text{univ}}$ has to be locally free as the end terms of the sequence are.
\end{example}
...
\begin{proof}
	...
	\]
	By \Cref{rem:scalars-on-ext}, scalar multiplication by $\lambda \neq 0$ does not change the isomorphism class of the middle term of the sequence, hence there exists a one-dimensional subspace $k \injectto \Ext^1(\mc{O}(a),\mc{O}^{\oplus m-1})$ such that each nonzero element corresponds to an exact sequence of the same form. Consider the associated closed embedding $\alpha \from \AA^1 \to \VV(\Ext^1(\mc{O}(a),\mc{O}^{\oplus m-1})\dual)$ and let $\mc{E}$ be the universal extension from \Cref{cor:universal-extension}. Then, the vector bundle $\pullb{(\id_{\PP^1}\times\alpha)}\mc{E}$ on $\PP^1\times\AA^1$ realizes the required specialization. 
\end{proof}

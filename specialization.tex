...
\begin{example}
	...
	such that for all nonzero rational points $\xi\in\AA^1 = \VV(\Ext^1(\mc{O}(2),\mc{O})\dual)$ we have the isomorphisms $\mc{E}_{\text{univ}}|_{\xi \times \AA^1} \simeq \mc{O}(1)\oplus\mc{O}(1)$ and $\mc{E}_{\text{univ}}|_{0\times \AA^1} \simeq \mc{O} \oplus \mc{O}(2)$. Note that $\mc{E}_{\text{univ}}$ has to be locally free as the end terms of the sequence are.
\end{example}
...
\begin{remark}
	By twisting the exact sequence in the proof of \Cref{specialization-partitions} and using the same argument, we see that for every integer $n$ and with $b_i, \pi$, and $b_I$ as above, the vector bundle $\bigoplus_{i=1}^m \mc{O} (b_i+n)$ specializes to $\bigoplus_{I\in\pi} \mc{O}(b'_I+n) \oplus \mc{O}(n)^{\oplus m-\abs{\pi}}$.
\end{remark}